...
logging.debug('Reading in data.txt')
file = open('data.txt', 'r')
fileContents = file.readlines()
file.close()
logging.debug('Done reading in data.txt')
...
for line in fileContents:
  print(prime_number_checker(int(line)))

logging.debug('Building output string from dictionary')
data = prime_number_checker(int(fileContents[0]))
print(data)

logging.debug('Attempting: writing to output.txt')
file = open('output.txt', 'w')
for line in fileContents:
  file.write(str(prime_number_checker(int(line)))+'\n')
file.close()
